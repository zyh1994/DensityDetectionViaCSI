...
class CSIStatus:
    ...
    def get_matrix(self):
        # convert the csi matrix to a one dimensional array, where each element is a complex number
        matrix = []

        for i in range(self.receiver_antennas):
            for j in range(self.transmitter_antennas):
                for k in range(self.number_of_tones):
                    matrix.append(str(self.csi_matrix[i][j][k]))
    ...
    def write(self, frame: bytes, length: int, timestamp: int):
        ...
        with open(self.filename, 'a', newline='') as csvfile:
            ...
            writer.writerow([csi.timestamp, csi.csi_length, csi.channel_number, csi.buffer_length, csi.payload_length,
                             ...
                             csi.rssi_antenna_2, ",".join(csi.get_matrix())])
