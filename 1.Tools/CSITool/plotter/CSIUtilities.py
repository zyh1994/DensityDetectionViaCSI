...
def db(val, ptype='pow'):
    if ptype == 'pow':
        return 10 * np.log10(val)
    elif ptype == 'mag':
        return 20 * np.log10(val)
    else:
        raise ValueError("Invalid ptype. Use either 'pow' or 'mag'.")


def get_total_rss(csi_st: Bfee):
...
def get_scaled_csi(csi_st: Bfee):

    # Pull out CSI
    csi = csi_st.get_payload_matrix()
